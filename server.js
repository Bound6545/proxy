...
// --- 2.5. SERVE BARE-MUX FILES FOR THE BROWSER ---
app.use(
  "/baremux/",
...
async function getClientId() {
  ...
  try {
    ...
    const scriptUrls = home.data.match(
      /<script crossorigin src="(https:\/\/a-v2\.sndcdn\.com\/assets\/[^"]+)"/g
    );
    ...
    if (q.startsWith("http")) {
      ...
      if (resolved && resolved.type === "users") {
        ...
        const playlistData = await axios.get(
          `https://api-v2.soundcloud.com/playlists/${resolved.id}?client_id=${cid}`
        );
      ...
      apiTarget = `https://api-v2.soundcloud.com/search/tracks?q=${encodeURIComponent(
        q
      )}&client_id=${cid}&limit=20`;
    }
